...
from ..encoders import small_cnn, cnn_with_pool
...
class FixedEye(BaseModule):
  ...
  def insert(simulation, **kwargs):
    ...
    eye.append(ET.Element("camera", name="fixed-eye", fovy="50"))

    # Add eye to a body
    body = kwargs.get("body", "thorax")
    if body == "worldbody":
  ...
  def get_observation(self, model, data, info=None):
    ...
    if self._buffer is not None:
      ...
      obs = np.concatenate([self._buffer[-1], self._buffer[-1] - self._buffer[0]], axis=0)
      # obs = np.concatenate([self._buffer[0], self._buffer[-1], self._buffer[-1] - self._buffer[0]], axis=0)

      # legacy code to check the visibility of target
      # import cv2
      # print("min", depth.min())
      # rand_i = np.random.randint(10)
      # im_np = np.zeros((np.concatenate((obs.shape[1:], np.array([3,])))))
      # im_np[:, :, 0] = obs[0, :, :]
      # cv2.imwrite(f"sample/a0{rand_i}.png", (im_np + 1.0) / 2 * 255)
      # im_np[:, :, 0] = obs[1, :, :]
      # cv2.imwrite(f"sample/a1{rand_i}.png", (im_np + 1.0) / 2 * 255)
      # im_np[:, :, 0] = obs[2, :, :]
      # cv2.imwrite(f"sample/a2{rand_i}.png", (im_np + 1.0) / 2 * 255)
      # im_np[:, :, 0] = obs[3, :, :]
      # cv2.imwrite(f"sample/a3{rand_i}.png", (im_np + 1.0) / 2 * 255)
    return obs
  ...
  @property
  def encoder(self):
    return cnn_with_pool(observation_shape=self._observation_shape, out_features=128)
    # return small_cnn(observation_shape=self._observation_shape, out_features=128)
