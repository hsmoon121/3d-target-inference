...
class BaseTrainer(ABC):
            ...
            with tqdm(total=step_per_iter, desc=f" Iter {iter}") as progress:
                for step in range(step_per_iter):
                    if train_mode == "online":
                        batch_args = self._get_online_batch(batch_sz)
                    elif train_mode == "offline":
                        batch_args = self._get_offline_batch(batch_sz)
                    else:
                        batch_args = self._get_replay_batch(batch_sz)
    ...
    def _get_online_batch(self, batch_sz):
        pass
    ...
    def parameter_recovery(
        ...
        for param_i in range(n_param):
            stat_i, traj_i = valid_data[0][param_i], valid_data[1][param_i]

            # Unsqueeze (as 1-size batch)
            stat_i = np.expand_dims(stat_i, axis=0)
            traj_i = np.expand_dims(traj_i, axis=0)
            map_fit = self.amortizer.infer(stat_i, traj_i, n_sample=n_sample, type=infer_type)
            inferred_params.append(map_fit)
