"""
For the agent simulation,
we borrow the code of user-in-the-box (https://github.com/aikkala/user-in-the-box)
Reference:
- Ikkala et al., Breathing Life Into Biomechanical User Models, UIST'22
"""
...
class Simulator(gym.Env):
  ...
  def _initialise(cls, config, simulator_folder, run_parameters):
    ...
    run_parameters["rendering_context"] = Context(model,
                                                  max_resolution=run_parameters.get("max_resolution", [800, 600]))
  ...
  def _initialise_action_space(self):
    ...
    num_actuators = self.bm_model.nu + self.perception.nu + self.task.nu
  ...
  def _initialise_observation_space(self):
    ...
    if "user_params" in observation:
      obs_dict["user_params"] = spaces.Box(dtype=np.float32, **self.bm_model.get_user_space_params())
  ...
  def step(self, action):
    ...
    self.perception.set_ctrl(self._model, self._data, action[self.bm_model.nu:self.bm_model.nu+self.perception.nu])

    # Set direct control for task (e.g. direct click without muskulo-simulation)
    self.task.set_ctrl(self._model, self._data, action[self.bm_model.nu+self.perception.nu:])
    ...
    effort_cost = self.bm_model.get_effort_cost(
      self._model,
      self._data,
      **self.bm_model.user_params,
    )
    reward -= effort_cost
  ...
  def get_observation(self, info=None):
    ...
    # Add any user-specific parameters
    user_params = self.bm_model.get_user_param(self._model, self._data)
    if user_params is not None:
      observation["user_params"] = user_params
  ...
  def reset(self):
    ...
    # Update user params in task
    if self.bm_model._with_user_params:
      self.task.update_user_param(self.bm_model.user_params)
    info = self.task.reset(self._model, self._data)

    return self.get_observation(info)
  
  def update_reward(self, linear_variable):
    self.task.update_reward(linear_variable)
